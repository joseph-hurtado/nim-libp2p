...
suite "BufferStream":
  ...
  test "push and wait":
    proc testPushTo(): Future[bool] {.async.} =
      ...
      check: 1 == await buff.readOnce(data)
  ...
  test "readExactly raises":
    proc testReadExactly(): Future[bool] {.async.} =
      ...
      var readFut = buff.readExactly(data)
      await buff.close()
  ...
  test "readOnce":
    proc testReadOnce(): Future[bool] {.async.} =
      ...
      var data: array[3, byte]
      let readFut = buff.readOnce(data)
      await buff.pushTo("123".toBytes())
      check buff.len == 3
  ...
  test "small reads":
    proc testWritePtr(): Future[bool] {.async.} =
      ...
      try:
        while true:
          let x = await buff.readOnce(data)
