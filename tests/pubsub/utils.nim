...
proc subscribeNodes*(nodes: seq[Switch]): Future[void] {.async.} =
  for dialer in nodes:
    for node in nodes:
...
proc subscribeSparseNodes*(nodes: seq[Switch], degree: int = 2): Future[void] {.async.} =
  if nodes.len < degree:
  ...
  for i, dialer in nodes:
    ...
    for node in nodes:
      if dialer.peerInfo.peerId != node.peerInfo.peerId:
        await dialer.connect(node.peerInfo)
        dialer.subscribePeer(node.peerInfo)

proc subscribeRandom*(nodes: seq[Switch]): Future[void] {.async.} =
  for dialer in nodes:
